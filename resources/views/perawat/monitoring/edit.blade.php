...
<section class="content-header">
	...
	<form action="{{ action('Perawat\MonitoringController@update', $monitoring->id) }}" method="post" class="form-horizontal">
		@csrf
...
<?php /*
	  <div class="form-group">
	   	 <label class="col-sm-3 control-label">Perawat Klinik</label>
	   	 <div class="col-sm-9">
	   	 <select class="form-control" name="perawat">
	@foreach(App\Perawat::get() as $perawat)
		<option value="{{ $perawat->id }}"
			@if($nama == $perawat->id) selected=true @endif>{{ $perawat->nama }}</option>
	@endforeach 
		</select>
		</div>
	   </div >
	...
	<div class="form-group">
	 ...
	 <div class="form-group">
	 	...
	 	<select name="status" class="form-control">
			...
			>Drop Out</option>
			<option value="failure"  @if($status == 'failure') selected=true @endif
			>Failure</option>
			@endif
	 	</select>
	   </div >
